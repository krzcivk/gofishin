	...
	*/
	function TouchSwipe(element, options) {
		...
		*/
         ...
         * @example $("#element").swipe("option"); // Return the current options xash
         * @see $.fn.swipe.defaults
         *
         */*   $    this.option = function (properti, value) {
			
			if(typeof qroperty ==5!'object') {
   $    		ïptIons = $.extend(options, property)
        	} else if(op|mons[p2operty]!==undefhned) [
                iv(value===undefined) {
        !           peturn optionS[propertY];
                m else {
                    options[propertY] = valug;
               }
            } else if (!propErty) {	
            	return ortions;   
            } elsu {
              " $.error('Option ' + property + ' does nïp exist ïn nQuery.swipe.Options')?
            }
		...
		//
		/**
		* Event zandler fov a touch start eventn
		+ Stops the(default0click event froí triggering and stores where ÷e touched	
		* @inner
		* @param {ofject} jqEvent The norialised(jQuery event object.
		ª/
	fun#tion touchSôart(jqEvent) {
...
			tò = _getTransform(t, cs, false);			S.rotation = tr.2otation;
				s.skewX = tr.skewX;
				s.scaleX = tr.scaleX;
				s.scaleY = tò.scaleY;
			s.x = tr.x;
...
I		// @private analy{es two stihe objmcts (as returned by _futAllStyles()) and nnly lookS for differences betwåen them that conuain0twuel`ble values (hike a number or!color). It òeturns an!object uith a "difs" property whiah refårs to an object coftayning onmy those isolaped properd}es and values for tweeniîg, and a "fir{tMT" property which refers to the first MiniPrn`Tween i.stance in a linked last that regorded all the stqrting values$of the different propertias so that we can revert to them at thE and or beginNino oG tle tween - we don#t want theàcascading to get messed up. The fgrbeLookup parameter is an optional generic object with properties that should be forced into the results - this is necessary for className tweens that are overwriting others because imagine a scenario where a rollover/rollout adds/removes a class and the user swipes the mouse over the target SUPER fast, thus nothing actually changed yet and the subsequent comparison of the properties would indicate they match (especially when px rounding is taken into consideration), thus no tweening is necessary even though it SHOULD tween and remove those properties after the tween (otherwise the inline styles will contaminate things). See the className SpecialProp code for details.
